...
def display_recognition_results(recognized_ids, imgDisplay, faceCurFrame, modeType, counter, imgModeList):
    font = cv2.FONT_HERSHEY_SIMPLEX
    font_scale = 1.0
    font_color = (255, 255, 255)  # White color for text
    box_color_recognized = (0, 255, 0)  # Green color for recognized faces
    box_color_unrecognized = (0, 0, 255)  # Red color for unrecognized faces
    thickness = 2
    line_height = 35  # Space between lines

    for (top, right, bottom, left), recognized_id in zip(faceCurFrame, recognized_ids):
        ...
        if recognized_id is not None:
            ...
            cv2.rectangle(imgDisplay, (left, top), (right, bottom), box_color_recognized, thickness)
            ...
            if student:
                ...
                cv2.putText(imgDisplay, f"{name}", (left, bottom + 20), font, font_scale, font_color, thickness)
        else:
            # Draw a red box around the unrecognized face
            cv2.rectangle(imgDisplay, (left, top), (right, bottom), box_color_unrecognized, thickness)
            # Optionally, display "Unknown person" text
            cv2.putText(imgDisplay, "Unknown person", (left, bottom + 20), font, font_scale, font_color, thickness)
...
def main():
    ...
    while cv2.getWindowProperty("Lion Vision", 0) >= 0:  # Check if the window is still open
        ...
        if faceCurFrame:
            ...
            display_recognition_results(recognized_ids, img, faceCurFrame, modeType, counter, imgModeList)
        else:
            modeType, counter = reset_mode(img, modeType, counter, imgModeList)

        # Display the resulting frame with any UI updates
        cv2.imshow("Lion Vision", img)
